        ...
        if isinstance(content_message, MessageContentText):
            if id in message_references:
                ...
                message_references[id] = cl.Message(
                    author=config['app_name'], content=content_message.text.value
                )
...
async def start_chat():
    ...
    await cl.Message(
        ...
        content=config['app_intro'],
        disable_feedback=True,
    ).send()


@cl.step(name=config['app_name'], type="run", root=True)
async def run(thread_id: str, assistant_id:str, human_query: str, file_ids: List[str] = []):
...
async def on_message(message_from_ui: cl.Message):
    ...
    assistant_id = assignAssistant.get_assistantID(message_from_ui.content)
    await run(
        thread_id=thread.id, assistant_id=assistant_id, human_query=message_from_ui.content, file_ids=files_ids
    )
